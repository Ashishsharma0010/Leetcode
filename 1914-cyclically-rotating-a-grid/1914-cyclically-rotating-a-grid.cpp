...
public:
    vector<vector<int>> rotateGrid(vector<vector<int>>& grid, int k) {
        ...
        for (int layer = 0; layer < layers; layer++) {
            ...
            // Rotate left by (k % len): element at index i goes to index i-shift
            int len = elems.size();
            int shift = k % len;
            
            // Rebuild rotated sequence
            vector<int> rotated(elems.begin() + shift, elems.end());
            rotated.insert(rotated.end(), elems.begin(), elems.begin() + shift);
